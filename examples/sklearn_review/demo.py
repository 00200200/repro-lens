"""Train scikit-learn in separate project copies and compare the recorded outputs."""
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path(".repro-lens/sklearn-demo"))
    args = parser.parse_args()
    args.output.mkdir(parents=True, exist_ok=True)
    directory = Path(tempfile.mkdtemp(prefix="run-", dir=args.output)).resolve()
    baseline = None
    summary = {}
    print(f"{'Scenario':16} {'verify':10} {'compare':10} training accuracy", flush=True)
    for name, expected, changed in [
        ...
        if name == "refactor":
            ...
            config.write_text(
                config.read_text(encoding="utf-8").replace("max_depth = 3", "max_depth = 1"),
                encoding="utf-8",
            )
        ...
        accuracy = report["runs"][0]["metrics"]["training_accuracy"]
        print(f"{name:16} {report['status']:10} {expected or '-':10} {accuracy:.2f}", flush=True)
